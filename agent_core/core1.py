...
def analyze(program_raw: str, manifest_raw: str) -> Analysis:
    ...
    return Analysis(result.verdict, result.reason, _digest(program), _digest(manifest), (
        "manifest and sensitivity labels issued by trusted host",
        "host manifest is immutable between analysis and execution",
        "agent has no unmediated external effect paths",
    ))


def simulate(program_raw: str, manifest_raw: str, prior: Analysis) -> list[tuple[str, str]]:
    """Preflight again and simulate sends; never trust a supplied verdict alone."""
    current = analyze(program_raw, manifest_raw)
